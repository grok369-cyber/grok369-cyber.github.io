...
// Holds the base64 data URL read from a file input, keyed by that
// input's id, until the form is submitted (or the upload is replaced).
const uploadedImageData = {};

/**
 * Wires an <input type="file"> up to a live preview and reads the
 * chosen image into a base64 data URL, stored in uploadedImageData.
 * Data URLs work great with localStorage since they're just strings —
 * no server or file storage needed.
 */
function initImageUpload(inputId, previewId, filenameId) {
  ...
  input.addEventListener("change", () => {
    ...
    // Keep localStorage usage sane — cap uploads around 3MB.
    if (file.size > 3 * 1024 * 1024) {
      showToast("Image is too large — please choose one under 3MB.", "warning");
      input.value = "";
      return;
    }
    ...
    reader.onload = () => {
      uploadedImageData[inputId] = reader.result; // base64 data URL
      if (preview) {
    ...
    reader.onerror = () => showToast("Couldn't read that image — please try another file.", "warning");
    reader.readAsDataURL(file);
  });
}

function resetImageUpload(inputId, previewId, filenameId) {
  delete uploadedImageData[inputId];
  const input = document.getElementById(inputId);
  const preview = document.getElementById(previewId);
  const filenameLabel = document.getElementById(filenameId);
...
function initAdminForms() {
  ...
  initImageUpload("blog-image-upload", "blog-image-preview", "blog-image-filename");
  initImageUpload("gallery-image-upload", "gallery-image-preview", "gallery-image-filename");

  if (blogForm) {
    blogForm.addEventListener("submit", (e) => {
      e.preventDefault();
      // An uploaded file takes priority over a pasted URL.
      const image = uploadedImageData["blog-image-upload"] || document.getElementById("blog-image").value.trim();
      const blogs = JSON.parse(localStorage.getItem("savio_blogs") || "[]");
      blogs.push({
        id: cryptoId(),
      ...
      blogForm.reset();
      resetImageUpload("blog-image-upload", "blog-image-preview", "blog-image-filename");
  ...
  if (galleryForm) {
    galleryForm.addEventListener("submit", (e) => {
      e.preventDefault();
      const image = uploadedImageData["gallery-image-upload"] || document.getElementById("gallery-image").value.trim();
      if (!image) {
        showToast("Please provide an image URL or upload a photo.", "warning");
        return;
      }
      ...
      galleryForm.reset();
      resetImageUpload("gallery-image-upload", "gallery-image-preview", "gallery-image-filename");
      renderGalleryGrid();
      renderGalleryManageList();
